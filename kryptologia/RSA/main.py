...
class GUI(QMainWindow, Ui_MainWindow):
    ...
    def generateKeys(self):
        keys = generateKeys()
        self.public_e.setText(str(keys['e']))
        self.public_n.setText(str(keys['n']))
        self.private_d.setText(str(keys['d']))
        self.private_n.setText(str(keys['n']))


    def cipher(self):
        public_e = int(self.public_e.text())
        public_n = int(self.public_n.text())
        text = removeDiacritics(self.input.toPlainText())
        self.output.setPlainText(str(encrypt(public_e, public_n, text)))

    def decipher(self):
        private_d = int(self.private_d.text())
        private_n = int(self.private_n.text())
        text = re.sub(r'[^0-9 ]', '', self.input.toPlainText())
        self.output.setPlainText(str(decrypt(private_d, private_n, text)))
    ...
    def __init__(self):
        ...
        self.setupUi(self)
        self.keysCreateButton.clicked.connect(self.generateKeys)
